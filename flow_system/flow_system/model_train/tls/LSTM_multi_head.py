...
class BiLSTM_Multi_attention(nn.Module):
    ...
    def __init__(self, input_size, hidden_dim, n_layers):
        ...
        self.f1= nn.Sequential(nn.Linear(256, 84),
                               nn.Dropout(0.1),
                               nn.ReLU())
    ...
    def forward(self, x):
        ...
        tem = final_hidden_state.permute(1,0,2)
        att_output, attention = self.f0(tem, tem, tem)
        # 加入attention机制

        # print("attn_output:", attn_output.shape)
        print(tem.shape)
        logit = self.f1(att_output)
        result = self.f2(logit)
