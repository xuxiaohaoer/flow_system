...
class featureType(object):
    ...
    def toImag(self):
        while len(self.content)<784:
            self.content.append(0)
        tem = self.content[:784]
        # tem = tem/255
        
        from feature_extract.models import image_feature
        import json
        t = image_feature(name=self.name, label=self.label, feature= json.dumps(tem))
        t.save()
        tem = np.array(tem).reshape(28, 28)
        return [tem, self.label, self.name]
