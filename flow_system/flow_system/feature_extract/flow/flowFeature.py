...
class featureType(object):
    def __init__(self):
        ...
        self.pk_s = 0  # 每秒包传输速度
        self.pk_src_s = 0  # 每秒上行包数++
        self.pk_dst_s = 0  # 每秒下行包数++
        self.time = 0  # 整体持续时间
        ...
        self.var_packetsize_packet = 0  # 方差包大小++
    ...
    def toList(self):
        ...
        self.max_time, self.min_time, self.mean_time, self.std_time, self.sum_time= cal(self.time_sequence)
        self.max_packetsize_flow, self.min_packetsize_flow, self.mean_packetsize_flow, self.std_packetsize_flow, self.sum_packetsize_flow= cal(
            self.packetsize_flow_sequence)
        self.max_time_flow, self.min_time_flow, self.mean_time_flow, self.std_time_flow, self.sum_time_flow = cal(self.time_flow_sequence)

        self.time_src_sequence = cal_seq(self.time_src_sequence)
        self.time_dst_sequence = cal_seq(self.time_dst_sequence)
     
        self.max_time_src, self.min_time_src, self.mean_time_src, self.std_time_src, self.sum_time_src = cal(
            self.time_src_sequence)
        self.max_time_dst, self.min_time_dst, self.mean_time_dst, self.std_time_dst, self.sum_time_dst = cal(
            self.time_dst_sequence)
        self.max_packetsize_src, self.min_packetsize_src, self.mean_packetsize_src, self.std_packetsize_src, self.sum_packetsize_src = cal(
            self.packetsize_src_sequence)
        self.max_packetsize_dst, self.min_packetsize_dst, self.mean_packetsize_dst, self.std_packetsize_dst, self.sum_packetsize_dst= cal(
            self.packetsize_dst_sequence)
        self.max_packetsize_packet, self.min_packetsize_packet, self.mean_packetsize_packet, self.std_packetsize_packet, self.sum_packetsize_packet= cal(
            self.packetsize_packet_sequence)
        self.var_packetsize_packet = np.var(self.packetsize_packet_sequence)
        ...
        self.pk_src_s = cal_div(self.num_src, self.time)
        self.pk_dst_s = cal_div(self.num_dst, self.time)

        self.max_entropy, self.min_entropy, self.mean_entropy, self.std_entropy, self.sum_entropy = cal(self.bitFre)
        self.max_cipher_app_entropy, self.min_cipher_app_entropy, self.mean_cipher_app_entropy, self.std_cipher_app_entropy, self.sum_cipher_app_entropy = cal(
            self.cipher_bitFre)
        ...
        from feature_extract.models import flow_feature
        import json
        feature = [self.pack_num, time, self.flow_num, ip_src, self.packetsize_size, self.dport,
                # 5
