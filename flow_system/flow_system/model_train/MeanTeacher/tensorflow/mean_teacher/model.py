...
class Model:
    ...
    def __init__(self, run_context=None):
        ...
        with tf.name_scope("placeholders"):
            self.images = tf.placeholder(dtype=tf.float32, shape=(None, 28, 28, 1), name='images')
        ...
        with tf.name_scope("objectives"):
            ...
            self.out_prediction, self.out_label = predictions_label(self.class_logits_1, self.labels)
        ...
        with tf.variable_scope("validation_metrics") as metrics_scope:
            self.metric_values, self.metric_update_ops = aggregate_metric_map({
                ...
                "eval/accuracy/1": streaming_accuracy(self.out_prediction, self.out_label),
                "eval/precision/1": streaming_precision(self.out_prediction, self.out_label),
                "eval/recall/1": streaming_recall(self.out_prediction, self.out_label),
            })
        ...
        self.result_formatter = string_utils.DictFormatter(
            order=["eval/error/ema", "error/1", "class_cost/1", "cons_cost/mt","accuracy/1","precision/1","recall/1"],
...
def tower(inputs,
    ...
    with tf.name_scope(name, "tower"):
        ...
        with \
            ...
            net = wn.conv2d(net, 32, scope="conv_1_1")
            net = slim.max_pool2d(net, [2, 2], scope='max_pool_1')
            assert_shape(net, [None, 14, 14, 32])
            net = wn.conv2d(net, 64, scope="conv_2_1")
            net = slim.max_pool2d(net, [2, 2], scope='max_pool_2')
            assert_shape(net, [None, 7, 7, 64])
            ...
            # net = wn.conv2d(net, 32, kernel_size=[1, 1], scope="conv_3_2")
            # net = slim.avg_pool2d(net, [7, 7], scope='avg_pool')
            # net = slim.dropout(net, 1 - dropout_probability, scope='dropout_probability_2')
            # assert_shape(net, [None, 1, 1, 32])
            ...
            # assert_shape(net, [None, 14, 14, 128])
...
def predictions_label(logits, labels, name=None):
    with tf.name_scope(name,"Precision") as scope:
        applicable = tf.not_equal(labels, -1)
        labels = tf.boolean_mask(labels, applicable)
        logits = tf.boolean_mask(logits, applicable)
        predictions = tf.argmax(logits, -1)
