...
def parse_ip_data(packet, nth, timestamp):
    ...
    if (len(data)!=0):
        ...
        if len(feature.content)<784:
            feature.content += packet[14:]
            ...
            if len(stream) > 6:
                if stream[2] == 1:  # sslv2 client hello
                    ...
                    while i < cipher_length:
                        ...
                        i += 3
                    if not feature.packet["client_hello"]:
                        feature.packet["client_hello"] = packet
...
def multiple_handshake(nth, buf, ip):
    i, n = 0, len(buf)
    msgs = []
                ...
                try:
                    ...
                    if record_type == 'handshake':
                        ...
                        if handshake_type == 11:  # certificate
                            ...
                            a = parse_tls_certs(nth, msg.data, msg.length, packet)
                            # return msgs, tot_len
...
def parse_tls_records(ip, stream, nth, nth_seq, timestamp):
    ...
    if bytes_used == 0:
        try:
            records, bytes_used = multiple_handshake(nth, stream, ip)
            flag = True
    ...
    for record in records:
        ...
        if record_type == 'handshake':
            ...
            if handshake_type == 2:  # server hello
                ...
                if (not feature.packet["server_hello"]):
                    packet = []
                    packet.append(nth.to_bytes(length=4, byteorder='big', signed=False))
                    # packet.append(math.floor(timestamp * 1000).to_bytes(length=4, byteorder='big', signed=False))
                    packet.append((len(ip) + 14).to_bytes(length=4, byteorder='big', signed=False))
                    ...
                    feature.packet["server_hello"] = packet
            ...
            if handshake_type == 11:  # certificate
                ...
                head += ip.data.dport.to_bytes(length = 2, byteorder = 'big', signed= False)
                a = []
                a = parse_tls_certs(nth, record.data, record.length, packet)
            ...
            if handshake_type == 1:  # sslv3 tlsv1 client hello
                ...
                try:
                    ...
                    pass
                    # print(feature.ip_dst)
                    # print(feature.ip_src)
                    # print(nth)
...
def parse_tls_certs(nth, data, record_length, packet):
    """
    ...
    try:
        ...
        if handshake.type == 11:  # TLS Certificate
            ...
            if not feature.packet["certificate"]:
                feature.packet["certificate"] = packet
            ans += certs

    return ans


def cut_bytes(tem, packet):
    i = 0
    while (i<len(tem)):
        if (i+4)<=len(tem):
    ...
    try:
        with open(filename, 'rb') as f:
            ...
            for timestamp, packet in capture:
                ...
                if not feature.packet["client_hello"] or not feature.packet["server_hello"] or not feature.packet["certificate"]:
                # if len(feature.content)<100:
...
def pre_flow_image(base_dir, save_dir, label):
    i = 0
    dataset = []
...
def main():
    ...
    read_file(base_dir + '192.168.56.33.pcap')
...
if __name__ == "__main__":
    ...
    print(time.asctime(time.localtime(time.time())))
    # with open(dir + "/"+ 'illustration.txt','w+',encoding='utf-8') as f:
    #     list = []
    #     list.append("be divided 255\n")
    #     list.append("client_hello:35\n")
    #     list.append("nth\n")
    ...
    #     list.append("length\n")
    #     list.append("version\n")
    #     list.append("cipher_suites\n")
    #     list.append("certificate:33\n")
    #     list.append("nth\n")
    #     # list.append("timestamp\n")
    #     list.append("ip_len\n")
    #     list.append("ip_src\n")
    ...
    #     list.append("type\n")
    #     list.append("length\n")
    #     list.append("cert_1_version\n")
    #     # list.append("cert_1_signature\n")
    #     list.append("cert_1_issue\n")
    #     list.append("cert_1_subject\n")
    #     list.append("cert_1_timebefore\n")
    #     list.append("cert_1_timeafter\n")
        
    #     list.append("cert_2_version\n")
    #     # list.append("cert_1_signature\n")
    #     list.append("cert_2_issue\n")
    #     list.append("cert_2_subject\n")
    #     list.append("cert_2_timebefore\n")
    #     list.append("cert_2_timeafter\n")
    #     list.append("label:0-black;1-white\n")
    #     list.append("name:ip_src-ip_dst\n")
    #     f.writelines(list)
    # f.close()
    #
