...
need_more_parse = False  # 需要更多TLS信息
need_more_certificate = False # 需要读取证书
need_http = True
...
def parse_ip_packet(eth, nth, timestamp):
    ...
    if isinstance(ip.data, dpkt.tcp.TCP) and payload:
        ...
        #     bitFre[key] += 1
        # feature.bitFre += bitFre
        # if bitFre.sum()!=0:
        #     bitFre /= bitFre.sum()
        # for key in bitFre:
        #     if key!= 0:
        #         entropy -= key *math.log(key, 2)
        # feature.entropy_seq.append(entropy)
...
def read_file(filename):
    """
    读取filename文件的pcap格式文件，若存在pcapng格式，请及时沟通
    """
    try:
...
def pre_flow(base_dir, save_dir, label):
    ...
    for filename in tqdm(os.listdir(base_dir)):
        ...
        feature.label = label
        # 根据返回格式选择合适feature函数，如toIamge()或toList()
        dataset.append(feature.toImage())

    dataset_np = np.array(dataset)
    if not os.exits(save_dir):
        os.mkdirs(save_dir)
...
if __name__ == "__main__":
    print("begin")
    # 演示
    pre_flow("data/eta_flow/train/black/", 'feature_flow/train_black.npy', 'black')
    pre_flow("data/eta_flow/train/white/", 'feature_flow/train_white.npy', 'white')
    pre_flow("data/eta_flow/test/black/", 'feature_flow/test_black.npy', 'black')
    pre_flow("data/eta_flow/test/white/", 'feature_flow/test_white.npy', 'white')
